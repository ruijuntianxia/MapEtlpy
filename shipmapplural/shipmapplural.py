...
try:
    ...
    sql='select a.* from scmp.v_tptshipmentu a order by  startdate ,shipment_no,GpsTime ' #sql语句 坐标点过多，oracle一个栏位无法存储，在转档时进行存储 
    rr=curs.execute (sql)
    ...
    #测试
    r = redis.Redis(host='127.0.0.1', port=6379,db=0,password='zh123')
